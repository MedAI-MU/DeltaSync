...
public final class Main {
    ...
    private static void runServer(String[] args) throws Exception {
        ...
        int    blockSz = parsePositiveInt(
                p.opt(List.of("-b", "--block-size"), String.valueOf(BlockAnalysis.DEFAULT_BLOCK_SIZE)),
                "--block-size");
    ...
    private static void runSync(String[] args) throws Exception {
        ...
        List<Integer> synced = DeltaSyncClient.sendDeltaSync(
                host, port, file, remoteName, blockSz, timeout, psk, !noBar);

        JSONArray arr = new JSONArray();
        for (Integer i : synced) arr.put(i.intValue());
        System.out.println(new JSONObject().put("synced_blocks", arr).toString(2));
    }

    private static int parsePort(String s) {
        int port = Integer.parseInt(s);
        if (port < 1 || port > 65535) {
            throw new IllegalArgumentException("Port must be in 1..65535: " + s);
        }
        return port;
    }
    ...
    private static int parsePositiveInt(String s, String name) {
        ...
        if (v <= 0) throw new IllegalArgumentException(name + " must be positive: " + s);
        return v;
    }
